...
| D-20 | Cross-encoder Rerank Floor (option A) — `src/retrieval/reranker.py` + Pass 0 trong `hybrid_search` | Direction 2 (retrieval precision): vá embedding semantic blindness ở disambiguation cấp Điều (P-08/Q022). Khi `rerank=True`: xếp lại pool dense (~50) theo cross-encoder `bge-reranker-v2-m3` (query, chunk) → Pass 0 dùng thứ tự này thay Dense Floor. **LOCAL $0 inference**; opt-in (`--rerank`), mặc định OFF. **MPS treo vô hạn** với model này (xác nhận hang 3.5h) → ép **CPU** (env `RERANKER_DEVICE` override). Vai trò kép: lớp rerank + "teacher" sinh hard-negative cho finetune embedding. Q022 N=1: cơ chế đúng (cross-encoder chấm "Điều 1 hạn mức"=0.926 >> "Điều 4 hiệu lực"=0.002 → loại đúng Điều 4 spurious, F1 Điều 0.40→0.50) NHƯNG F1 Khoản vẫn 0.00 (lỗi sâu cấp Khoản/Điểm còn lại). **Full ablation 26 câu ±rerank CHƯA chạy** (cần API). | 2026-06-23 |
...
reranker.py          ← question + candidates [{text}] (cross-encoder, direction 2, D-20)
                     ...
                     → semantic_filter Pass 0 "Rerank Floor" khi hybrid_search(rerank=True)
                       (run_pipeline(rerank=), cờ --rerank) — mặc định OFF
