...
3. Decision Log trong file này (D-01…D-29) — đặc biệt D-23 (implements đa-cha), D-24 (multi-LLM), D-27 (Gemini-only + không nuốt lỗi API), D-28 (corpus lao-dong), **D-29 (truy hồi ≠ chọn trích dẫn — nút thắt nay ở bộ sinh)**
...
| D-29 | **Tách rời "truy hồi" và "chọn trích dẫn"** — giữ `[:REFERS_TO]` (Pass 3), **BÁC BỎ `budget_mode="fill"`**; nút thắt hiện tại là bộ sinh, không phải truy hồi | Đợt cải tiến sau phản biện nâng **độ bao phủ** rất mạnh (v2 137 câu: 0,738→0,873 **+0,136 · 28T/0B**; v4 28 câu: 0,762→0,929 **+0,167 · 9T/0B**, gap3 0,49→0,85) nhưng **F1 sinh lại GIẢM −0,070** trên chính tập v4. Phép tách 5 nhánh chỉ đích danh nguyên nhân: (C) chỉ nới trần token 12000 = **0,000 khác biệt** — trên v4 ngữ cảnh dài nhất chỉ 5146 token nên trần 6000 CHƯA TỪNG cắn, giả thuyết "trần token là thủ phạm" SAI; (A) chỉ `REFERS_TO` = −0,002 (CI chứa 0) nhưng **lãi đúng chỗ**: gap3 +0,041, gap4 +0,067, lỗ ở gap1/gap2 nơi bao phủ vốn đã 1,00 → **GIỮ**; (D) thêm pool 100 + tắt độ hiếm = −0,029 (CI chứa 0); (E) **chỉ `fill` = −0,115, CI95 [−0,195; −0,038], 3T/16B** — tệ hơn cả gói đầy đủ, mà độ bao phủ chỉ **+0,000**. Điểm quyết định: `fill` làm **recall CŨNG giảm** (−0,018) và NormR −0,036 → KHÔNG phải đánh đổi precision lấy recall mà là **đơn vị sai lấn chỗ đơn vị đúng** (bỏ trần per_norm cho phép 1 văn bản chiếm hết ngân sách). Quan hệ gần tuyến tính: **mỗi +1 trích dẫn ≈ −0,03 F1**; hệ trích 2,71 khi đáp án chuẩn chỉ cần 1,57 → bộ sinh trích theo *lượng ngữ cảnh nhận được*, không theo *nhu cầu câu hỏi*. Hệ quả: ngữ cảnh tốt hơn KHÔNG tự chuyển thành F1 tốt hơn; bước tiếp là tiết chế trích dẫn ở bộ sinh (tinh chỉnh trên v2, v4 chỉ chạy MỘT lần để giữ tư cách tập kiểm thử). `fill`/`graph` giữ nguyên trong code CHỈ để tái lập kết quả âm. Tiền lệ D-12/D-19/D-20 | 2026-08-15 |
