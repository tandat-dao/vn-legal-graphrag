...
Cặp *terracotta + nền kem ấm* là một hướng rất phổ biến hiện nay và **không phải** bản sắc thật của dichvucong.gov.vn (cổng thật dùng xanh dương/đỏ cờ). Thay vì chốt cứng một phương án trên giấy, `index.html` **cài sẵn cả hai** và đổi được ngay trên thanh trạng thái (hai nút `A` / `B`) — để so trực tiếp trên máy chiếu thật rồi mới chốt.
...
o-cau-hoi · o-juris · o-mode · o-verify · nut-hoi
...
| **Thanh trạng thái** | Tầng 2 (4.1) — **chỉ mượn hình thức, không mượn chức năng** | Trang mới **không có nav**. Dùng dải `var(--chrome-bg-2)` cao 48px làm **thanh trạng thái**. Phần trạng thái chạy là **chỉ-đọc**: badge chế độ (`#badge-mode`) + tốc độ phát lại (đọc từ `replay_speed` của `/api/mode`). Thứ **duy nhất** bấm được ở đây là **công tắc bảng màu A/B** (mục 1.3) — nó chỉ đổi lớp trình bày, không đụng pipeline, nên bấm được cả khi đang chạy. Badge "PHÁT LẠI" = nền `#FFC251` chữ đen `font-bold` (nổi bật, không thể bỏ sót — spec mục 4.2 bắt buộc). Badge "TRỰC TIẾP" = nền `white/15` chữ trắng. **KHÔNG dựng nút chuyển live⇄replay** — backend chưa có endpoint đổi mode lúc chạy (`DEMO_MODE` đọc từ env một lần khi khởi động `ui/server.py`, `/api/mode` chỉ `GET` trả trạng thái), nên nút đó sẽ là **nút chết** = đúng thứ mục 7.3 cấm. Việc đổi mode nóng thuộc **Task 6**; khi backend có endpoint thì mới thêm. **Tuyệt đối không dựng nav giả với link chết.** |
| **Ô nhập câu hỏi** | Hero (4.3) + SearchBar (4.4) | Dải hero `var(--chrome-bg)` có radial gradient, bên trong là card trắng bo 12px chứa `textarea` + 2 `select` + checkbox Verifier + nút Hỏi. Nút Hỏi: `bg-[var(--terracotta)]`, hover `--terracotta-dk`, disabled `bg-[var(--gray-300)]`. |
| **Chip gợi ý câu hỏi** (`#goi-y`) | 3 nút CTA vàng (4.3) | Đổi từ nút to sang **chip**: nền `#FFC251`, chữ đen, `text-xs font-semibold px-3 py-1.5`, bo 4px, hover `#F0B340`. Số lượng bằng số fixture — hợp với hàng 3 cột cũ. |
| **Thẻ 7 bước** (`#cac-buoc`) | Thẻ 4.5 + nhãn vạch accent | Mỗi bước = một thẻ trắng bo 12px, viền `--border-card`. Nhãn: `<div class="w-1 h-5 bg-terracotta rounded-full">` + số bước + tên `uppercase tracking-wider text-navy`. Trạng thái: `.buoc-cho{opacity:.45}` · `.buoc-chay{box-shadow:0 0 0 2px var(--terracotta)}` *(thay ring indigo)* · `.buoc-xong{opacity:1}` + dấu ✓ màu `#28A745`. Mốc thời gian `t` in `text-[11px] text-[var(--text-muted)]` bên phải. |
...
| **Panel nguyên văn** (`#panel-nguyen-van`) | Thẻ + header widget (4.7) | Card trắng bo 12px, header nền `var(--navy-faint)` chữ `--navy`. Tên văn bản hiển thị đầy đủ, **slug in dưới bằng `font-mono text-[11px] text-[var(--text-muted)]`** (spec mục 3.3 bắt buộc có đường tra slug gốc). |
...
| **Cảnh báo fixture** (`#canh-bao-fixture`) | — | Nền `#FFF7E6`, viền dưới `#FFC251`, chữ `var(--gray-900)`, `text-xs px-6 py-2`. |
...
Giữ nguyên lưới hiện có: `grid xl:grid-cols-[minmax(0,1fr)_380px] gap-4`, panel phải `sticky top-16`. Chỉ **đổi `max-w-[1600px]` → giữ nguyên** — 1170px của repo cũ quá hẹp cho stepper + đồ thị + panel; đây là ví dụ điển hình của "không ép theo UI cũ".
